...
import { getByIdDoctors, RegisDoctor, UpdatedDoctors } from '../../../store/api/doctors';
...
export default function Register() {
    ...
    const { register, handleSubmit, reset, control, formState: { errors, isSubmitting }, setValue } = useForm({
    ...
    const getDoctors = async () => {
        const response = await getByIdDoctors(id as string);
        if (response) {
            reset({
                name: response?.name,
    ...
    const onSubmit = async (event: Doctor) => {
        const res = id ? await UpdatedDoctors(event, id as string) : await RegisDoctor(event)
        ...
        switch (res?.statusText) {
            ...
            case "Created":
                setOpen(true)
                setAlertTxt("Médico criado com sucesso.")
                setAlertType('success')
                break;
            case "No Content":
                setOpen(true)
                setAlertTxt("Médico atualizado com sucesso.")
                setAlertType('success')
                break;
        }
    }
