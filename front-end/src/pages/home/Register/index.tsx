...
import { getByIdPatient, RegisPatient, UpdatedPatient } from '../../../store/api/patient';
...
export default function RegisterPatient() {
    ...
    const { handleSubmit, reset, control, formState: { errors, isSubmitting }, setValue } = useForm({
    ...
    const getPatient = async () => {
        const response = await getByIdPatient(id as string);
        if (response) {
            reset({
                name: response?.name,
    ...
    const onSubmit = async (event: Patients) => {
        const res = id ? await UpdatedPatient(event, id as string) : await RegisPatient(event)
        switch (res?.statusText) {
            ...
            case "Created":
                setOpen(true)
                setAlertTxt("Paciente criado com sucesso.")
                setAlertType('success')
                break;
            case "No Content":
                setOpen(true)
                setAlertTxt("Paciente atualizado com sucesso.")
                setAlertType('success')
                break;
        }
    }
                        ...
                        <Box display={'flex'} alignItems={'center'} gap={2}>
                            ...
                            <Box>
                                <Typography component="h1" variant="h4">
                                    {id ? "Editar" : "Cadastro"} de Paciente
                                </Typography>
                                <Typography
                                    color="textSecondary"
                                    gutterBottom
