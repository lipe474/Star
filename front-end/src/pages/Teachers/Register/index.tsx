...
import { getByIdTeachers, RegisTeachers, UpdatedTeachers } from '../../../store/api/teachers';
...
export default function Register() {
    ...
    const onSubmit = async (event: Teacher) => {
        const res = id ? await UpdatedTeachers(event, id as string) : await RegisTeachers(event)
        ...
        switch (res?.statusText) {
            ...
            case "Created":
                setOpen(true)
                setAlertTxt("Docente criado com sucesso.")
                setAlertType('success')
                break;
            case "No Content":
                setOpen(true)
                setAlertTxt("Docente atualizado com sucesso.")
                setAlertType('success')
                break;
        }
    }
    ...
    const getTeachers = async () => {
        const response = await getByIdTeachers(id as string);
        if (response) {
            reset({
                name: response?.name,
                ethnicity: response?.ethnicity,
                titration: response?.titration,
                                    ...
                                    <Controller
                                        render={({
                                            ...
                                            <TextField
                                                ...
                                                value={value}
                                                onChange={onChange}
                                                label={<Required text={"Titulação"} />}
                                                error={errors.titration !== undefined}
                                                helperText={errors.titration?.message}
                                            />
                                        )}
                                        name="titration"
                                        control={control}
